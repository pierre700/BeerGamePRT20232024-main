...
def partie (nbRounds, startStock, typeDemande, party):
    ...
    for a in range(nbRounds+1) :
        ...
        ID=["a","b","c","d"]
        for k in range(4):
            Data[i].append(updateStock(Data[i], Buffer[i], nextBuffer[i], prevBuffer[i], prevStock[i], prevRetard[i], Stock[i], Retard[i], currentRound, dmdCli, i==3, typeDemande, ID[i], listInput[i], party))
...
def updateStock(roleData, buffer, nextBuffer, prevBuffer, prevStock, prevRetard, Stock, Retard, currentRound, dmdCli, condRetailer, typeDemande, ID, orderRound, party):  
    order = orderfct(buffer, currentRound)
    #if prevStock == 0 :
        #print("=0", order, prevStock)
    if order > prevStock and prevStock is not False:
        if ID == "d":
            print(ID)
        print("=0", order, prevStock)
