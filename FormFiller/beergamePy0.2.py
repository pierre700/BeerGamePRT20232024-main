...
def partie (nbRounds, startStock, typeDemande, party):
    ...
    for a in range(nbRounds+1) :
        ...
        ID=["a","b","c","d"]
        for k in range(4):
            Data[k].append(updateStock(Data[k], Buffer, nextBuffer[k], prevBuffer[k], prevStock, prevRetard[k], Stock[k], Retard[k], currentRound, dmdCli, k==3, typeDemande, ID[k], listInput[k], party, k))
    ...
    #wholeData.pop()
    #retailData.pop()
        print(listPlayer[k], Data[k], Buffer[k])
    #print('distr', distrData, distrBuffer)
    #print('whole', wholeData, wholeBuffer)
    #print('retail', retailData, retailBuffer)
    return Data
...
def updateStock(roleData, buffer, nextBuffer, prevBuffer, prevStock, prevRetard, Stock, Retard, currentRound, dmdCli, condRetailer, typeDemande, ID, orderRound, party, k):  
    ...
    for i in range(k+1):
        if orderList[k-i] > prevStock[k-i] and prevStock[k-i] is not False:
            if ID == "d":
                print(ID)
            #print("=0", order, prevStock)
            #print("order", "-", "prevStock", order, "-", prevStock)
            a = orderList[k-i]
            #diff = orderList[k-i] - prevStock[k-i] - a
            order = prevStock[k-i] + a
            #prevRetard = prevRetard + diff
    if orderList[k] > prevStock[k] and prevStock[k] is not False :
        a = orderList[k]
        diff = orderList[k] - prevStock[k] - a
        prevRetard = prevRetard + diff
    else : 
        order=orderList[k]
    if nextBuffer != False : 
